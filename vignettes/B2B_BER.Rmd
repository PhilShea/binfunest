...
That was unsatifying; the answer is almost identical to the version including the zeros.  Plotting the data on a linear scale indicates why.
...
That result indicates that the B2B is actually needed.  We can see what is hapenning here by looking at the data.  

```{r}
Q8Sbits
```

The 14 dB SNR observation has onely one symbol error, but reports two bit errors.  This is because the 8QAM Star constellation has a few adjacent symbols that have two bit changes, and that one error doubles the estmiated error rate.  Therefore, this estimator indicates that the BER is not binomial distributed.
