...
inline_hook <- function (x) {
  if (is.numeric(x)) {
    ...
    res <- ifelse(x == round(x) && x < .Machine$integer.max,
      sprintf("%d", x),
      sprintf("%.2G", x)) # end ifelse
    paste(res, collapse = ", ") # else if(is.numeric...
  }
}
...
In order to use all of the data, and to derive statistical significance from the estimates, we must turn to maximum likelihood estimation.  This will model the parameters which based on those that *most likely* created the observed data (including the zeros).
...
Those are pretty close estimates (although not as close as the prior fits). The diagonal of the inverse of the Hessian matrix is an estimate of the variance of the parameters, so their square root is the standard deviation. The expression below will show how many standard deviations from the answers those estimates are.
...
The covariance indicates that there is a relationship between the two parameters.  The [correlation matrix]( https://en.wikipedia.org/wiki/Covariance_matrix#Relation_to_the_correlation_matrix) can be computed as follows (we make a function as we'll find it useful):

```{r}
mle.corr <- function( mle) {
  cov <- vcov(mle)
  sqrt( sqrt( diag(cov)) %*% cov %*% diag( cov))
}
mle.corr( mle3)
```
...
The following will simulate an additional run at 19 dB, and add that data to the current observations.  We need to make a vector of the number of trials now as they are not all the same. The plot below has an extended range.
