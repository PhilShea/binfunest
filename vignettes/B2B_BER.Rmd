...
In high performance communications systems it is common for both the transmitter (Tx) and receiver (Rx) to add noise to the signal. Unless you have a perfect example of one or the other, it is impossible to measure the source: both the Tx and Rx will contribute. This has the effect of having a minimum BER even when no noise is added to the signal, called the Back-to-Back (B2B) BER, and is often expressed in Decibels as the SNR which would the B2B BER in a perfect Rx.
...
$$ P_{B2B}(\gamma) = P\left[ Off\left( \frac{1}{1/\gamma + 1/B}\right)\right]$$

where $Off$ is the linear offset (i.e., `undB( offset)`), and $B$ is the linear B2B SNR.

## Creating BER Functions with B2B & Offset

If we have a function $BER( s_{dB})$ of the SNR in Decibels, we can express the B2B and offset in decibels too.

$$
P_e( s_{dB}, B_{dB}, O_{dB}) = BER( -dB( undB( -s_{dB}) + undB( -B_{dB}) - O_{dB})
$$

The `B2BQ` package provides a function factory to convert any such function into a function with B2B and Offset in Decibels.
...
plot( s, y=QPSKdB( s), typ="l", log="y", ylim=c( 1e-10, 0.5), 
      ...
      ylab="BER")
lines( s, y=QPSKdB.B2B( s, Inf, O1), col="blue")
...
bplot1 <- function( s, r, N, O, B, ylim=c( 1e-10, 0.5)) {
  ...
  points( s, r/N)
  lines( s, y=QPSKdB( s))
  lines( s, y=QPSKdB.B2B( s, Inf, O), col="blue")
  lines( s, y=QPSKdB.B2B( rep( B1, length( s)), Inf, O1), col='black', lty=2)
  legend( "bottomleft",
        legend=c( "Data", "Theory", "Offset", "Offset + B2B", "B2B"),
        lty=c( NA, 1, 1, 1, 2), pch=c( 1, NA, NA, NA, NA),
...
That was unsatifying; the answer is almost identical to the version including hte zeros.  Plotting the data on a linear scale indicates why.
...
```{r}
 plot( s, y=QPSKdB.B2B( s, B1, O1), col='red', type='l', panel.first = grid(),
  ...
  legend( "bottomleft",
        ...
        lty=c( NA, 1, 1, 1, 1), pch=c( 1, NA, NA, NA, NA),
...
The same result can be reached with this package's `mleB2B` function, which facilitates the data being hosted in a data frame or list:
...
summary(mle4)
mle4coef <- coef(mle4)
plot(  s, r2/N, log='y', panel.first = grid(), ylim=c(1e-10, 0.5))
lines( s, y=QPSKdB( s), col='blue')
lines( s, y=QPSKdB.B2B( s, B2, O2), col="red")
lines( s, y=QPSKdB.B2B( s, mle4coef[1],  mle4coef[2]), col="green")
legend( "bottomleft",
        legend=c( "Data", "Theory", "3 dB Offset + 80 dB B2B",  
                  "Estimated"),
        lty=c( NA, 1, 1, 1), col=c( 'black', 'blue', 'red', 'green'),
        pch=c( 1, NA, NA, NA))
```

Typically the B2B parameter will have a very large variance in these instances, and multiple runs will demonstrate this.  In instances like this, the data do not support a B2B parameter, and one should estimate the offset only using the `fixed` option of `mle` or simply setting the B2B parameter.  Note that when optimising over a single variable, a few other changes are required.  
...
The lower AIC for `mle4` begs the question that `mle5` is a statistically better estimate.

This is interesting in that the process is detecting the 80 dB B2B even though no errors were detected past 13 dB SNR.  The example below will show the same process with an infinite B2B, showing essentially no difference.

```{r noB2B}
(r3 <- rbinom( length( s), N, QPSKdB( s - O2)))
mle6 <- mleB2B( N=N, Errors=r3, f=QPSKdB.B2B, fparms=list( x=s),
                start=c(b2b=20, offset=0))
summary(mle6)
mle7 <- mleB2B( N=N, Errors=r3, f=QPSKdB.B2B, fparms=list( x=s, B2B=+Inf),
...
Since we had simulated 21 runs of `r N` trials, we can do one more run of `r 21*N` trials at a high SNR. There were no errors starteing at 14 dB SNR, so we will try this at 15 dB.

```{r}
N2 <- 21 * N
Nvec <- c( rep(N, length(s)), N2)
s2 <- c(s, 15)
(r4 <- c( r2, rbinom( 1, N2, QPSKdB.B2B( 15, B2, O2))))
mle8 <- mleB2B(  Errors=r4, N=Nvec, f=QPSKdB.B2B,
...
We can get the coefecients from the following expression:
